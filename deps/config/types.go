...
func DefaultCurioConfig() *CurioConfig {
	return &CurioConfig{
		...
		HTTP: HTTPConfig{
			...
			RetrievalMaxParallelRequests: NewDynamic(256),
			BulkRetrieval: BulkRetrievalConfig{
				MaxConcurrentStreams: NewDynamic(32),
				MaxBlocks:            NewDynamic(4096),
				MaxWindow:            NewDynamic(256),
				DefaultWindow:        NewDynamic(64),
				AdvertisedMaxStreams: NewDynamic(8),
				MergeGapKiB:          NewDynamic(1024),
				MaxRangeMiB:          NewDynamic(32),
				ResolveParallelism:   NewDynamic(64),
			},
		},
	}
}
...
type HTTPConfig struct {
	...
	RetrievalMaxParallelRequests *Dynamic[int]

	// BulkRetrieval tunes the bulk block retrieval endpoint (/aurora/bulk/v0).
	BulkRetrieval BulkRetrievalConfig
}

// BulkRetrievalConfig tunes the bulk block retrieval endpoint
// (POST /aurora/bulk/v0/blocks): batched block requests served from merged
// sequential piece reads. All values are dynamic — updates to the layered
// config apply to running nodes without a restart.
type BulkRetrievalConfig struct {
	...
	AdvertisedMaxStreams *Dynamic[int]
	...
	ResolveParallelism *Dynamic[int]
}
