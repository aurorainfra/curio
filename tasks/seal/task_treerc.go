...
func (t *TreeRCTask) CanAccept(ids []harmonytask.TaskID, engine *harmonytask.TaskEngine) ([]harmonytask.TaskID, error) {
	...
	err := t.db.Select(ctx, &tasks, `
		SELECT p.task_id_tree_c, p.sp_id, p.sector_number, l.storage_id FROM sectors_sdr_pipeline p
			...
			WHERE task_id_tree_c = ANY ($1) AND l.sector_filetype = 4
		UNION ALL
...
`, indIDs)
	...
	acceptables := make(map[harmonytask.TaskID]bool)
	for _, id := range ids {
		acceptables[id] = true
	}
	var acceptedIDs []harmonytask.TaskID
	for _, task := range tasks {
		if !acceptables[task.TaskID] {
			continue
		}
		for _, l := range ls {
			if string(l.ID) == task.StorageID {
				acceptedIDs = append(acceptedIDs, task.TaskID)
				break
			}
		}
	}
